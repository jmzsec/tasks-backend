    ...
    stages {
       ...
       stage ('Frontend - Trivy Scanner') {
            steps {
              ...
              //  sh "docker run --rm -v ${HOME}/Library/Caches:/root/.cache/ aquasec/trivy $registry:$BUILD_NUMBER"
                sh "docker run --rm --skip-download aquasec/trivy $registry_front:$BUILD_NUMBER"
       ...
       stage ('Backend - Trivy Scanner') {
            steps {
              ...
              //  sh "docker run --rm -v ${HOME}/Library/Caches:/root/.cache/ aquasec/trivy $registry:$BUILD_NUMBER"
                sh "docker run --rm --skip-download aquasec/trivy $registry_back:$BUILD_NUMBER"
        ...
        stage('DAST - OWASP ZAP') {
            steps {
                sh 'docker run -v $PWD/artifacts:/zap/wrk -t owasp/zap2docker-weekly zap-baseline.py -t http://192.168.224.185:9999 -I -r OWASPZAP.html'
